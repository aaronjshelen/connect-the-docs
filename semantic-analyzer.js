...
class SemanticAnalyzer {
  ...
  async getEmbeddings(texts) {
    ...
    if (textsToFetch.length > 0) {
      try {
        ...
        console.warn(
          "Failed to fetch embeddings, using fallback similarity:",
          error
        );
        // Use fallback for missing embeddings
        for (let i = 0; i < indices.length; i++) {
          const originalIndex = indices[i];
          results[originalIndex] = this.createFallbackEmbedding(
            textsToFetch[i]
          );
        }
      }
  ...
  async analyzeThemeSemantics(themes) {
    ...
    if (!Array.isArray(themes)) {
      console.warn(
        "analyzeThemeSemantics received non-array:",
        typeof themes,
        themes
      );
      return {
  ...
  async findConceptualConnections(document1Themes, document2Themes) {
    ...
    const themeTexts = allThemes.map((t) =>
      typeof t === "string" ? t : t?.label || t?.term || String(t)
    );
    ...
    for (let i = 0; i < themes1.length; i++) {
      for (let j = themes1.length; j < allThemes.length; j++) {
        ...
        if (similarity >= 0.7) {
          // Threshold for conceptual similarity
          connections.push({
            theme1: themes1[i],
    ...
    return connections.sort(
      (a, b) => b.semanticSimilarity - a.semanticSimilarity
    );
  }
  ...
  async analyzeDocumentRelationship(doc1, doc2) {
    ...
    const doc1Themes = Array.isArray(doc1.themes)
      ? doc1.themes
      : doc1.themes instanceof Set
      ? Array.from(doc1.themes)
      : [];
    const doc2Themes = Array.isArray(doc2.themes)
      ? doc2.themes
      : doc2.themes instanceof Set
      ? Array.from(doc2.themes)
      : [];
    const doc1Defs = Array.isArray(doc1.definitions)
      ? doc1.definitions
      : doc1.definitions instanceof Set
      ? Array.from(doc1.definitions)
      : [];
    const doc2Defs = Array.isArray(doc2.definitions)
      ? doc2.definitions
      : doc2.definitions instanceof Set
      ? Array.from(doc2.definitions)
      : [];
    ...
    const semanticConnections = await this.findConceptualConnections(
      doc1Themes,
      doc2Themes
    );
    ...
    const relationshipScore = this.calculateRelationshipScore({
      ...
      avgSemanticSimilarity:
        semanticConnections.length > 0
          ? semanticConnections.reduce(
              (sum, conn) => sum + conn.semanticSimilarity,
              0
            ) / semanticConnections.length
          : 0,
      contentSimilarity,
      themeOverlap:
        exactThemeMatches.length /
        Math.max(doc1Themes.length, doc2Themes.length),
      defOverlap:
        exactDefMatches.length / Math.max(doc1Defs.length, doc2Defs.length),
    });
  ...
  calculateTFIDFSimilarity(text1, text2) {
    ...
    const vec1 = Array.from(allWords).map(
      (word) => words1.filter((w) => w === word).length
    );
    const vec2 = Array.from(allWords).map(
    ...
    for (let i = 0; i < themes.length; i++) {
      ...
      if (cluster.length > 1) {
        clusters.push({
          ...
          category: this.inferClusterCategory(
            cluster.map((idx) => themes[idx])
          ),
        });
      }
    }
  ...
  async buildThemeHierarchy(themes, embeddings) {
    ...
    for (let i = 0; i < themes.length; i++) {
      ...
      for (let j = 0; j < themes.length; j++) {
        ...
        const otherText =
          typeof otherTheme === "string" ? otherTheme : otherTheme.label;

        const similarity = this.cosineSimilarity(embeddings[i], embeddings[j]);
        if (
          similarity > maxSimilarity &&
          this.isMoreGeneral(otherText, themeText)
        ) {
  ...
  findExactMatches(arr1, arr2) {
    ...
    const normalized1 = array1.map((item) =>
      typeof item === "string"
        ...
        : (item.label || item.term || "").toLowerCase().trim()
    );
