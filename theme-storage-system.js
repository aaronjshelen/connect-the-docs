...
class ThemeStorageSystem {
  ...
  linkDocumentToTheme(docId, themeId, confidence = 1.0, context = "") {
    ...
    this.linkMetadata.set(linkKey, { confidence, context, createdAt: new Date() });
  ...
  getRelatedDocuments(docId, minSharedThemes = 1, includeDefinitions = true) {
    ...
    return Array.from(relatedDocs.entries())
      .filter(([_, relation]) => 
        relation.sharedThemes.size + relation.sharedDefinitions.size >= minSharedThemes
      )
      .map(([docId, relation]) => ({
        documentId: docId,
        document: this.documents.get(docId),
        sharedThemes: Array.from(relation.sharedThemes).map(id => this.themes.get(id)),
        sharedDefinitions: Array.from(relation.sharedDefinitions).map(id => this.definitions.get(id)),
        connectionStrength: relation.score,
        totalShared: relation.sharedThemes.size + relation.sharedDefinitions.size
      }))
  ...
  getSharedThemes(minDocuments = 2) {
    return Array.from(this.themes.values())
      ...
      .map(theme => ({
        ...theme,
        documents: Array.from(theme.documents).map(id => this.documents.get(id)),
  ...
  findSimilarTheme(label, normalizedLabel) {
    ...
    for (const theme of this.themes.values()) {
      const similarity = this.calculateSimilarity(normalizedLabel, theme.normalizedLabel);
  ...
  findCommonDocuments(themeIds) {
    const themeSets = Array.from(themeIds).map(id => 
      this.documentsByTheme.get(id) || new Set()
    );
  ...
  export() {
    return {
      ...
      indices: {
        themesByDocument: Array.from(this.themesByDocument.entries()).map(([k, v]) => [k, Array.from(v)]),
        documentsByTheme: Array.from(this.documentsByTheme.entries()).map(([k, v]) => [k, Array.from(v)]),
        definitionsByDocument: Array.from(this.definitionsByDocument.entries()).map(([k, v]) => [k, Array.from(v)]),
        documentsByDefinition: Array.from(this.documentsByDefinition.entries()).map(([k, v]) => [k, Array.from(v)])
      },
      metadata: {
        nextId: this.nextId,
        exportedAt: new Date()
